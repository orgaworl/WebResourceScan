...
def plot_top_domains_global(df: pd.DataFrame, out_dir: str, n: int = 25) -> None:
    ...
    ax.legend(handles=legend_handles, frameon=False, fontsize=12, loc="lower right")
...
def plot_domain_presence_heatmap(df: pd.DataFrame, out_dir: str, n: int = 20) -> None:
    ...
    ax.set_xticklabels(cats, rotation=35, ha="right", fontsize=13)
...
def plot_tp_count_vs_resources_bubble(df: pd.DataFrame, out_dir: str) -> None:
    ...
    leg1 = ax.legend(handles=cat_handles, frameon=False, fontsize=11, loc="upper left",
                     title="Site Category", title_fontsize=12, ncol=2)
    ax.add_artist(leg1)

    # Size legend
    for sr, label in [(0.1, "10%"), (0.5, "50%"), (1.0, "100%")]:
        ax.scatter([], [], s=sr * 300, color="#888888", alpha=0.6, label=f"Script ratio {label}")
    ax.legend(frameon=False, fontsize=11, loc="lower right", title="Script Ratio", title_fontsize=12)
